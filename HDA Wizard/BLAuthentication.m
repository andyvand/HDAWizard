...
- (BOOL)isAuthenticated:(NSString *)forCommand {	
	...
	[forCommand getCString:command maxLength:1024 encoding:NSUTF8StringEncoding];
...
- (int)getPID:(NSString *)forProcess {
    ...
    do {
        ...
        tmpbytes = [tempData mutableBytes];
        len = fread(tmpbytes, 1, 512, outpipe);
        if( len > 0 ) {
...
-(BOOL)executeCommand:(NSString *)pathToCommand withArgs:(NSArray *)arguments {
    ...
    err = ([self runProcessAsAdministrator:pathToCommand withArguments:arguments output:nil errorDescription:nil] == NO);
	
	if(err != YES)
    {
		NSLog(@"Error in runProcessAsAdministrator");
...
-(BOOL)executeCommandSynced:(NSString *)pathToCommand withArgs:(NSArray *)arguments {
    return [self executeCommand:pathToCommand withArgs:arguments];
}
